...
createFile :: Pool Connection -> UUID -> Maybe UUID -> Maybe Text -> IO File
createFile pool projectId mParentId mName = withResource pool $ \conn -> do
  [file] <-
    query
      conn
      "INSERT INTO files (id, project_id, parent_folder_id, name, type) \
      \VALUES (gen_random_uuid(), ?, ?, ?, 'file') \
      ...
      (projectId, mParentId, fromMaybe "Untitled File" mName)
  return file
...
deleteFile pool fileId = withResource pool $ \conn ->
  void $ execute conn "DELETE FROM files WHERE id = ?" (Only fileId)
